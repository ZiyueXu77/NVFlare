...
class FedReviveConfig:
    """Configuration class for FedRevive algorithm parameters.

    This class configures the FedRevive-specific parameters including
    knowledge distillation and staleness handling.

    Attributes:
        kd_enabled: Whether to enable knowledge distillation augmentation. Default: True
        kd_buffer_size: Size of the KD buffer for storing recent updates. Default: 3
        kd_beta: Weight for combining original update with KD result (0-1). Default: 0.5
        kd_temperature: Temperature for KD softmax. Default: 1.0
        kd_learning_rate: Learning rate for KD optimization. Default: 0.01
        kd_num_iters: Number of KD optimization iterations. Default: 10
    """
...
class ModelManagerConfig:
    """Configuration class for the model manager in FedRevive.

    This class configures how the model manager handles model updates, versioning,
    and aggregation strategies for FedRevive federated learning workflow.
...
class DeviceManagerConfig:
    """Configuration class for the device manager in FedRevive.
...
class SimulationConfig:
    """Configuration class for simulation settings in FedRevive.

    This class configures the simulated devices for testing federated learning
    pipelines with FedRevive algorithm.
...
class EdgeFedReviveRecipe(Recipe):
    """Recipe class for cross-edge federated learning using FedRevive algorithm.

    This class provides a high-level interface for configuring cross-edge
    federated learning jobs with the FedRevive algorithm. It configures the necessary components
    including model managers with KD augmentation, device managers, evaluators, and device simulation settings.

    FedRevive enhances asynchronous federated learning with:
    - Knowledge distillation to handle model staleness
    - Adaptive weighting based on update staleness
    ...
    Attributes:
        ...
        fedrevive_config: Configuration for FedRevive algorithm (optional)
    ...
    def __init__(
        ...
        model_manager_config: ModelManagerConfig,
        device_manager_config: DeviceManagerConfig,
        fedrevive_config: FedReviveConfig = None,
        ...
        self.fedrevive_config = fedrevive_config if fedrevive_config else FedReviveConfig()
    ...
    def create_job(self) -> EdgeJob:
        """Create a new EdgeJob instance for cross-edge federated learning with FedRevive.
    ...
    def _configure_job(self, job: EdgeJob):
        """Configure the edge job with all necessary FedRevive components.

        This method sets up the job with evaluators, simulation settings,
        client configurations, and server-side components including the
        FedRevive model manager with KD augmentation, device managers, and assessors.
        ...
        # add persistor, FedRevive model_manager, and device_manager
        persistor_id = job.to_server(PTFileModelPersistor(model=self.model), id="persistor")

        # Use FedReviveModelManager instead of BuffModelManager
        model_manager = FedReviveModelManager(
            ...
            kd_enabled=self.fedrevive_config.kd_enabled,
            kd_buffer_size=self.fedrevive_config.kd_buffer_size,
            kd_beta=self.fedrevive_config.kd_beta,
            kd_temperature=self.fedrevive_config.kd_temperature,
            kd_learning_rate=self.fedrevive_config.kd_learning_rate,
            kd_num_iters=self.fedrevive_config.kd_num_iters,
        )
