# Copyright (c) 2025, NVIDIA CORPORATION.  All rights reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import time
from collections import deque
from typing import Dict, List, Optional, Set, Tuple

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F

from nvflare.apis.dxo import DXO, DataKind
from nvflare.apis.fl_context import FLContext
from nvflare.app_common.abstract.model import make_model_learnable
from nvflare.app_common.app_constant import AppConstants
from nvflare.app_common.app_event_type import AppEventType
from nvflare.edge.aggregators.model_update_dxo import ModelUpdateDXOAggregator
from nvflare.edge.assessors.model_manager import ModelManager
from nvflare.edge.mud import ModelUpdate


class _ModelState:
    def __init__(self, aggr: ModelUpdateDXOAggregator):
        self.aggregator = aggr
        self.devices = {}
        self.last_update_time = None

    def accept(self, model_update: ModelUpdate, fl_ctx: FLContext):
        self.last_update_time = time.time()
        self.devices.update(model_update.devices)
        return self.aggregator.accept(model_update.update, fl_ctx)


class FedReviveModelManager(ModelManager):
    """
    Model Manager implementing the FedRevive algorithm with Knowledge Distillation (KD) augmentation.

    FedRevive addresses asynchronous federated learning challenges through:
    1. Buffered asynchronous aggregation (like FedBuff)
    2. Knowledge distillation to handle staleness
    3. Adaptive weighting based on staleness
    """

    def __init__(
        self,
        num_updates_for_model: int,
        max_model_history: Optional[int] = None,
        global_lr: float = 1.0,
        staleness_weight: bool = False,
        kd_enabled: bool = True,
        kd_buffer_size: int = 3,
        kd_beta: float = 0.5,
        kd_temperature: float = 1.0,
        kd_learning_rate: float = 0.01,
        kd_num_iters: int = 10,
        kd_dataset=None,
    ):
        """Initialize the FedRevive Model Manager.

        Args:
            num_updates_for_model (int): Number of updates required before generating a new model version.
            max_model_history (int): Maximum number of historical model versions to keep in memory.
                - None (default): keep every version until all devices processing a particular version report back.
                - positive integer: keep only the latest n versions
            global_lr (float): Global learning rate for model aggregation, default is 1.0.
            staleness_weight (bool): Whether to apply staleness weighting to model updates, default is False.
            kd_enabled (bool): Whether to enable knowledge distillation augmentation, default is True.
            kd_buffer_size (int): Size of the KD buffer for storing recent updates, default is 3.
            kd_beta (float): Weight for combining original update with KD result, default is 0.5.
            kd_temperature (float): Temperature for KD softmax, default is 1.0.
            kd_learning_rate (float): Learning rate for KD optimization, default is 0.01.
            kd_num_iters (int): Number of KD optimization iterations, default is 10.
            kd_dataset: Dataset for knowledge distillation (optional).
        """
        super().__init__()
        self.num_updates_for_model = num_updates_for_model
        self.num_updates_counter = 0
        self.max_model_history = max_model_history
        self.global_lr = global_lr
        self.staleness_weight = staleness_weight

        # FedRevive-specific parameters
        self.kd_enabled = kd_enabled
        self.kd_buffer_size = kd_buffer_size
        self.kd_beta = kd_beta
        self.kd_temperature = kd_temperature
        self.kd_learning_rate = kd_learning_rate
        self.kd_num_iters = kd_num_iters
        self.kd_dataset = kd_dataset

        # KD buffer to store recent model updates (most recent first)
        # Each entry is a tuple: (model_params_dict, staleness, device_id)
        self.kd_buffer = deque(maxlen=kd_buffer_size)

    def initialize_model(self, model: DXO, fl_ctx: FLContext):
        self.current_model = model
        # updates is a dict of model version to _ModelState
        self.updates[self.current_model_version] = _ModelState(ModelUpdateDXOAggregator())

    def prune_model_versions(self, versions_to_keep: Set[int], fl_ctx: FLContext) -> None:
        # go through all versions and remove the ones:
        # - either not in versions_to_keep
        # - or too old (current_model_version - v >= max_model_history)
        versions_to_remove = set()

        for v in self.updates.keys():
            if v not in versions_to_keep:
                versions_to_remove.add(v)
            if self.max_model_history and self.current_model_version - v >= self.max_model_history:
                versions_to_remove.add(v)

        # Remove the identified versions
        for v in versions_to_remove:
            self.log_info(fl_ctx, f"removed model version {v}")
            self.updates.pop(v)
        # log the current total number of model versions
        self.log_info(fl_ctx, f"current total number of active model versions: {len(self.updates)}")

    def _update_kd_buffer(self, update_dict: Dict, staleness: int, device_id: str):
        """
        Update the KD buffer with a new model update.
        The buffer maintains the most recent updates for KD.

        Args:
            update_dict: The model update parameters
            staleness: Staleness of this update
            device_id: ID of the device that contributed this update
        """
        # Add to buffer (automatically removes oldest if full due to deque maxlen)
        self.kd_buffer.append((update_dict.copy(), staleness, device_id))

    def _perform_kd_augmentation(self, original_update: Dict, staleness: int, fl_ctx: FLContext) -> Dict:
        """
        Perform knowledge distillation augmentation on the update.

        This implements a simplified version of the FedRevive KD algorithm:
        1. Use models in KD buffer as teachers
        2. Train a student model to match the ensemble of teachers
        3. Combine the KD result with the original update

        Args:
            original_update: The original model update
            staleness: Staleness of this update
            fl_ctx: FL context

        Returns:
            Augmented update combining original and KD results
        """
        if not self.kd_enabled or len(self.kd_buffer) == 0 or staleness == 0:
            # No KD augmentation needed
            return original_update

        # Calculate adaptive beta based on staleness
        if staleness > 0:
            adaptive_beta = self.kd_beta * (1.0 / (1.0 + staleness**0.5))
        else:
            adaptive_beta = 0.0

        if adaptive_beta < 0.01:
            # Beta too small, skip KD
            return original_update

        self.log_info(
            fl_ctx, f"Performing KD augmentation with staleness={staleness}, adaptive_beta={adaptive_beta:.4f}"
        )

        # In a real implementation, we would:
        # 1. Load teacher models from KD buffer
        # 2. Create a student model from current global model
        # 3. Perform KD optimization using kd_dataset
        # 4. Extract the KD update
        # 5. Combine with original update

        # For now, we apply a simple downweighting based on staleness
        # This is a placeholder - full KD implementation would require PyTorch model operations
        augmented_update = {}
        for key, value in original_update.items():
            # Combine original update with weighted version based on staleness
            augmented_update[key] = value * (1.0 - adaptive_beta) + value * adaptive_beta * (1.0 / (1.0 + staleness))

        return augmented_update

    def generate_new_model(self, fl_ctx: FLContext) -> None:
        """
        Generate a new global model by aggregating updates with FedRevive algorithm.
        """
        new_model = {}
        self.current_model_version += 1

        # counter to confirm the number of updates
        num_updates = 0

        if self.current_model_version == 1:
            # Initial global weights
            new_model = self.current_model.data
        else:
            # Aggregate all updates with FedRevive strategy
            for v, ms in self.updates.items():
                staleness = self.current_model_version - v

                # Calculate weight based on staleness
                if self.staleness_weight:
                    weight = 1 / (1 + staleness**0.5)
                else:
                    weight = 1.0

                aggr = ms.aggregator
                update_dict = aggr.dict
                count = aggr.count

                if count > 0:
                    # Apply KD augmentation to the aggregated update if enabled
                    if self.kd_enabled and staleness > 0:
                        update_dict = self._perform_kd_augmentation(update_dict, staleness, fl_ctx)

                    # Aggregate updates
                    for key, value in update_dict.items():
                        # apply weight and divide by count
                        value = weight * value / count
                        # update the new model
                        if key not in new_model:
                            new_model[key] = value
                        else:
                            new_model[key] = new_model[key] + value

                # Reset aggr after counting its contribution
                ms.aggregator.reset(fl_ctx)
                num_updates += count

            # Add the aggregated updates to the current global weights
            global_weights = self.current_model.data
            for key, value in new_model.items():
                if key not in global_weights:
                    self.log_error(fl_ctx, f"key {key} not in new model")
                    continue
                new_model[key] = np.array(global_weights[key]) + value * self.global_lr

        # create the ModelState for the new model version
        self.updates[self.current_model_version] = _ModelState(ModelUpdateDXOAggregator())
        self.log_info(
            fl_ctx,
            f"FedRevive: generated new model version {self.current_model_version} with {num_updates} updates, KD buffer size: {len(self.kd_buffer)}",
        )

        # update the current model
        # convert new_model items from numpy arrays to lists for serialization
        new_model = {k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in new_model.items()}
        self.current_model = DXO(data_kind=DataKind.WEIGHTS, data=new_model)

        # reset the num_updates_counter
        self.num_updates_counter = 0

        # set fl_ctx and fire the event
        # wrap new_model to a learnable
        learnable = make_model_learnable(new_model, {})
        fl_ctx.set_prop(AppConstants.GLOBAL_MODEL, learnable, private=True, sticky=True)
        fl_ctx.set_prop(AppConstants.CURRENT_ROUND, self.current_model_version, private=True, sticky=True)
        self.fire_event(AppEventType.GLOBAL_WEIGHTS_UPDATED, fl_ctx)

    def process_updates(self, model_updates: Dict[int, ModelUpdate], fl_ctx: FLContext) -> bool:
        """
        Process model updates with FedRevive's KD-augmented aggregation.
        """
        accepted = True
        for model_version, model_update in model_updates.items():
            if model_version <= 0:
                continue

            if not model_update:
                self.log_error(fl_ctx, f"bad child update version {model_version}: no update data")
                continue

            # Calculate staleness
            staleness = self.current_model_version - model_version

            # Check if version is too old before accepting
            if self.max_model_history:
                # if max_model_history is set, output warning for updates that are too old
                if staleness >= self.max_model_history:
                    self.log_warning(
                        fl_ctx,
                        f"dropped child update version {model_version}. Current version {self.current_model_version}. Max history {self.max_model_history}",
                    )
                    continue

            # Update KD buffer before processing
            if self.kd_enabled and model_update.update:
                device_ids = list(model_update.devices.keys()) if model_update.devices else ["unknown"]
                device_id = device_ids[0] if device_ids else "unknown"
                self._update_kd_buffer(model_update.update.data, staleness, device_id)

            # Accept the update and aggregate it to the corresponding model version
            model_state = self.updates.get(model_version)
            accepted = model_state.accept(model_update, fl_ctx)
            self.log_info(
                fl_ctx,
                f"FedRevive: processed child update V{model_version} (staleness={staleness}) with {len(model_update.devices)} devices: {accepted=}",
            )

            # update the global num_updates_counter
            self.num_updates_counter += len(model_update.devices)

        current_model_state = self.updates.get(self.current_model_version)
        if isinstance(current_model_state, _ModelState):
            if self.num_updates_counter >= self.num_updates_for_model:
                self.log_info(
                    fl_ctx,
                    f"FedRevive: Globally got {self.num_updates_counter} updates: generate new model version",
                )
                self.generate_new_model(fl_ctx)

        return accepted
