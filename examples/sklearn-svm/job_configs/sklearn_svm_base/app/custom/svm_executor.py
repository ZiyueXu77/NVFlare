...
class SVMExecutor(Executor):
    def __init__(
        ...
        self.local_model_output_path = local_model_output_path
        self.learner: Optional[SVMLearner] = None
        self.fl_ctx = None
    ...
    def log_values(self, key_values: dict, step):
        if self.writer:
            self.writer.add_scalas(key_values, step)
            self.writer.flush()

    def log_value(self, key, value, step):
        if self.writer:
            self.writer.add_scala(key, value, step)
            self.writer.flush()
