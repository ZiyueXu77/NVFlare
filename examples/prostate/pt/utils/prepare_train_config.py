...
def main():
    parser = argparse.ArgumentParser(description="generate train_config for prostate")
    ...
    with open(os.path.join(args.config_dir, "config_train.json"), "w") as outfile:
        json.dump(config_train, outfile, indent=4)

    with open(
        os.path.join(args.config_dir, "config_fed_server.json")
    ) as config_fed_server_file:
        ...
        for i in range(len(config_fed_server["components"])):
            if config_fed_server["components"][i]["id"] == "aggregator":
                config_fed_server["components"][i]["args"]["aggregation_weights"] = {
                    args.site_pre + str(site_id): 1.0
                    for site_id in range(1, args.num_sites + 1)
                }

    with open(os.path.join(args.config_dir, "config_fed_server.json"), "w") as outfile:
        json.dump(config_fed_server, outfile, indent=4)
