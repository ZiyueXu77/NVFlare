...
class SupervisedTrainer(Executor):
        ...
        for epoch in range(self.aggregation_epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                self.writer.add_scalar(
                    "train_loss", loss.item(), epoch_len * self.epoch_global + i
                )
            if val_freq > 0 and epoch % val_freq == 0:
                acc = self.local_valid(
                    self.valid_loader, "val_acc_local_model", abort_signal
                )
    ...
    def _train(
        ...
        global_acc = self.local_valid(
            self.valid_loader, "val_acc_global_model", abort_signal
        )
        if abort_signal.triggered:
            return make_reply(ReturnCode.TASK_ABORTED)
    ...
    def _submit_model(
        ...
        if model_data:
            ...
            # Set return code.
            self.log_error(
                fl_ctx, f"best local model not found at {self.best_local_model_file}."
            )
    ...
    def execute(
        ...
        if task_name == self.train_task_name:
            return self._train(
                shareable=shareable, fl_ctx=fl_ctx, abort_signal=abort_signal
            )
